...
The **Orchestrator** generates a JSON routing plan on every request to dynamically assign the execution graph and tool whitelists. The shared context is passed via reference. All state changes are emitted to Redis pub/sub and streamed to the SSE client.
